...
#From the boxplots it appears that the mean pain for drug A is lower than that for drugs B and C. Next, aov() can be used for fitting ANOVA models. The general form is 
...
#Exercise 1
#We will use the anova() function to perform ANOVA test in R. This function accepts a model object as input which corresponds a set of values based on a factor column as our treatment variable. Generally, if the measurements and treatments are in a dataframe called ‘our_data’ 
...
#1 - create a dataframe with two columns, observations and groups. 
...
#checking how group_numbers looks like: group_numbers
...
#2-	Investigate if Tanner levels have any significant effect on amount of insulin-like growth factor igf1.
